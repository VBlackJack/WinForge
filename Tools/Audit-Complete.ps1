...
$moduleNames = @('Core','InstallationEngine','ApplicationDatabase','ProfileManager','EnvironmentDetection','Prerequisites','SystemConfig','StartMenuLayout','StartMenuPinning','StartupManager','Win11ForgeGUI')

foreach ($mod in $moduleNames) {
    $hasTest = Test-Path "./Tests/$mod.Tests.ps1"
...
$issues = @(Invoke-ScriptAnalyzer -Path . -Recurse -Severity Warning,Error 2>$null | Where-Object { $_.ScriptPath -notmatch '\\Tests\\' })
...
$todos = @(Get-ChildItem -Path . -Include '*.ps1','*.psm1' -Recurse | Where-Object { $_.FullName -notmatch '\\Tests\\' } | Select-String -Pattern 'TODO|FIXME')
...
$hasGitHubActions = Test-Path "./.github/workflows"
$hasAzurePipelines = Test-Path "./azure-pipelines.yml"
if ($hasGitHubActions) {
...
$testsMissing = ($moduleNames | Where-Object { -not (Test-Path "./Tests/$_.Tests.ps1") }).Count
